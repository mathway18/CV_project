...
\texttt{GitHub: https://github.com/mathway18/CV\_project}}
...
\end{enumerate}
This gate is essential for avoiding quality collapse when Part 2 is already near-optimal.

\section{Experiments}
\subsection{Datasets and Protocol}
We evaluate on three mandatory datasets: a wild corridor video and two sample videos (\textit{bmx-trees}, \textit{tennis}). Following practical constraints, we use no-reference diagnostics for Part 2 vs. Part 3 comparison: boundary seam score (lower is better) and temporal warp error (lower is better). We also report accepted/rejected frame counts from the quality gate.

\subsection{Quantitative Results}
Table~\ref{tab:main} shows that Part 3 improves both seam and temporal metrics on \textit{bmx-trees} and \textit{tennis}, while preserving the corridor result exactly via full rejection.

\begin{table}[t]
\centering
\small
\caption{Part 2 vs. Part 3 on mandatory datasets (lower is better for seam and warp).}
\label{tab:main}
\begin{tabular}{lrrrr}
\toprule
Dataset & P2 Seam & P3 Seam & P2 Warp & P3 Warp \\
\midrule
bmx-trees & 18.2920 & \textbf{18.1038} & 5.3090 & \textbf{5.1043} \\
tennis & 3.7167 & \textbf{3.5840} & 2.1920 & \textbf{2.1745} \\
wild corridor & 3.1025 & 3.1025 & 1.4674 & 1.4674 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{Quality-gate behavior in Part 3.}
\label{tab:gate}
\begin{tabular}{lrrr}
\toprule
Dataset & Frames & Accepted & Rejected \\
\midrule
bmx-trees & 80 & 15 & 65 \\
tennis & 70 & 53 & 17 \\
wild corridor & 192 & 0 & 192 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Qualitative Analysis}
Figure~\ref{fig:qual} provides qualitative comparison placeholders to be replaced by exported frames from
\texttt{outputs/part2\_*/ppt\_frames} and \texttt{outputs/part3\_*/keyframes}.

\begin{figure}[t]
\centering
\fbox{\parbox[c][2.8cm][c]{0.47\linewidth}{\centering (a) Part 2 restored frame\\(replace with real frame)}}
\hfill
\fbox{\parbox[c][2.8cm][c]{0.47\linewidth}{\centering (b) Part 3 gated result\\(replace with real frame)}}
\\[2mm]
\fbox{\parbox[c][2.8cm][c]{0.47\linewidth}{\centering (c) Zoom-in seam region\\(replace with crop)}}
\hfill
\fbox{\parbox[c][2.8cm][c]{0.47\linewidth}{\centering (d) Temporal comparison\\(replace with frame pair)}}
\caption{Qualitative comparison template for mandatory datasets.}
\label{fig:qual}
\end{figure}

On \textit{bmx-trees}, Part 3 reduces visible seam transitions near high-texture background regions after object removal. On \textit{tennis}, improvements are smaller but consistent in both boundary smoothness and temporal continuity. On the wild corridor video, forcing refinement can introduce unnecessary edits; the quality gate avoids this and keeps the stronger Part 2 output unchanged.

\subsection{Discussion}
The results validate a practical principle: in post-processing for video restoration, \emph{selective refinement} is safer than aggressive global modification. The gate turns Part 3 into an adaptive optimizer rather than a mandatory transformation stage.

\section{Conclusion}
We presented a complete video object removal project that progresses from classical computer-vision baselines to modern foundation-model-based restoration and an adaptive Part 3 extension. The final system is reproducible, covers all mandatory datasets, and provides measurable gains where refinement is beneficial.

\textbf{Limitations.}
Our quantitative analysis for mandatory data is no-reference and therefore indirect. The diffusion backend is optional and not always feasible in limited hardware settings. Also, detector/SAM failures may still propagate into inpainting.

\textbf{Future Work.}
Future directions include: (1) evaluating on DAVIS with GT-based IoU/PSNR/SSIM at larger scale; (2) upgrading mask quality with stronger video foundation models (e.g., SAM 3-style concept prompts); and (3) learning a data-driven acceptance policy to replace hand-tuned quality thresholds.
