...
\begin{itemize}
    \item seam does not regress;
    \item temporal error does not exceed a configured regression bound;
    \item masked-region drift remains under a safety threshold.
\end{itemize}
...
\caption{Real frame-level qualitative comparisons extracted from final outputs. The figure shows three dataset panels arranged horizontally. Within each panel image, top-left is Part 2, top-right is Part 3, bottom-left is Part 2 zoom, and bottom-right is Part 3 zoom (same timestamp). \textit{bmx-trees} and \textit{tennis} show cleaner transitions around removal regions; wild corridor remains visually equivalent due to full gate rejection.}
\label{fig:qual-main}
\end{figure*}
...
\begin{itemize}
    ...
    \item \textbf{Texture vs. smoothness tension.} Masked texture can decrease even when seam/warp improve, so final judgment should combine quantitative diagnostics with visual inspection (Figure~\ref{fig:qual-main}).
\end{itemize}
